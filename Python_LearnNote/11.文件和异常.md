...
if __name__ == '__main__':
    main() except FileNotFoundError as e:
        print('指定的文件无法打开.')
    except IOError as e:
        print('读写文件时出现错误.')
    print('程序执行结束.')
...
我们如果希望把一个李彪或者字典中的数据保存到文件中又该怎么做呢？我们可以将数据以JSON格式进行保存，因为JSON表示出来就是一个字符串，可以被所有语言读取，也可以方便地通过网络传输给别人。JSON不仅是标准的数据交换格式，在Python中可以方便地使用它来表示结构化的数据，我们把变量或者对象转换成JSON格式的字符串之后，就可以将JSON写入文件中，相反，如果我们从文件中读取JSON，我们就可以把JSON字符串变成变量或者对象。下面的代码演示了如何使用`json`模块存储和读取字典对象。下面是一个JSON的简单例子。
...
| True / False                           | true / false |
| None                                   | null         |
我们可以使用Python中的json模块来对JSON数据进行编解码
...
+ `dups`-将Python对象处理成JSON格式的字符串
+ `load`-将文件中的JSON数据反序列化成对象
+ `loads`-将字符串的内容反序列化成Python对象
  
这里出现了两个概念，一个叫序列化，一个叫反序列化。自由的百科全书维基百科上对这两个概念是这样解释的:序列化（serialization）在计算机科学的数据处理中，是指将数据结构或对象状态转换为可以存储或传输的形式，这样在需要的时候能够恢复到原先的状态，而且通过序列化的数据重新获取字节时，可以利用这些字节来产生原始对象的副本（拷贝）。与这个过程相反的动作，即从一系列字节中提取数据结构的操作，就是反序列化

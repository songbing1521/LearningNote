...
JSON是一种轻量级的数据交换格式，在Python3中可以利用`json`对JSON格式进行编解码。

+ `json.dumps()`将Python对象编码成JSON字符串
+ `json.loads()`将已编码的JSON字符串解码为Python对象

![Python编码为JSON](../img/PythontoJSON.png)
![JSON解码为Python](../img/JSONtoPython.png)

### JSON语法规则

JSON是JavaScript对象表示法的子集。
...
1. 对象：大括号`{}`保存的对象是一个无序的键值对集合，键值对使用逗号`,`进行分割，键和值之间使用冒号`:`进行分割。键必须是字符串，值可以是任意类型的值。
2. 数组：中括号`[]`保存的数组是一个有序的值的列表，值之间使用逗号`,`进行分割。

## XML

XML是被用来传输和存储数据的。我们可以将数据从HTML中剥离出来，通过XML进行存储，利用JavaScript进行提取。
...
现在我们回到电子邮件，假设我们自己的电子邮件地址是<me@163.com>，对方的电子邮件地址是<friend@sina.com>（注意地址都是虚构的哈），现在我们用Outlook或者Foxmail之类的软件写好邮件，填上对方的Email地址，点“发送”，电子邮件就发出去了。这些电子邮件软件被称为MUA：Mail User Agent——邮件用户代理。
...
。
注意到构造MIMEText对象时，第一个参数就是邮件正文，第二个参数是MIME的subtype，传入'plain'表示纯文本，最终的MIME就是'text/plain'，最后一定要用utf-8编码保证多语言兼容性。我们用set_debuglevel(1)就可以打印出和SMTP服务器交互的所有信息。SMTP协议就是简单的文本命令和响应。login()方法用来登录SMTP服务器，sendmail()方法就是发邮件，由于可以一次发给多个人，所以传入一个list，邮件正文是一个str，as_string()把MIMEText对象变成str
...
with open('test.jpeg', 'rb') as f:
    ...
    mime = MIMEBase('image', 'jpeg', filename='test.jpeg')
    # 加上必要的头信息:
...
msg = MIMEMultipart()
msg['From'] = _format_addr('Python爱好者 <%s>' % from_addr)
msg['To'] = _format_addr('管理员 <%s>' % to_addr1) + ',' +_format_addr('管理员 <%s>' % to_addr2)
msg['Subject'] = Header('来自SMTP的问候……', 'utf-8').encode()

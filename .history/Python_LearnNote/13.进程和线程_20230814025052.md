...
Pool的默认大小是CPU的核数，如果要改变默认大小，可以在创建Pool的时候，传入一个`processes`参数：

## 子进程

很多时候，子进程并不是自身，而是一个外部进程。我们创建了子进程后，还需要控制子进程的输入和输出。下面例子演示了如何在Python代码中运行命令`nslookup www.python.org`，这和命令行直接运行的效果是一样的：
...
我们在这章讨论了多进程和多线程，现在我们来讨论一下这两种方式的优缺点。
...
## 分布式进程
[分布式进程](https://www.liaoxuefeng.com/wiki/1016959663602400/1017631559645600)受制于条件，暂时不做讲解。
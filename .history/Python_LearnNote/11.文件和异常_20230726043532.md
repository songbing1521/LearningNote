...
我们如果希望把一个李彪或者字典中的数据保存到文件中又该怎么做呢？我们可以将数据以JSON格式进行保存，因为JSON表示出来就是一个字符串，可以被所有语言读取，也可以方便地通过网络传输给别人。JSON不仅是标准的数据交换格式，在Python中可以方便地使用它来表示结构化的数据，我们把变量或者对象转换成JSON格式的字符串之后，就可以将JSON写入文件中，相反，如果我们从文件中读取JSON，我们就可以把JSON字符串变成变量或者对象。下面的代码演示了如何使用`json`模块存储和读取字典对象。
